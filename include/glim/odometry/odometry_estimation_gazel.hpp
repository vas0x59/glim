 ...
 */
struct OdometryEstimationGazelParams {
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW

  OdometryEstimationGazelParams();
  virtual ~OdometryEstimationGazelParams();
 ...
 */
class OdometryEstimationGazel : public OdometryEstimationBase {
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW

  OdometryEstimationGazel(std::unique_ptr<OdometryEstimationGazelParams>&& params);
  virtual ~OdometryEstimationGazel() override;
...
protected:
  ...
  virtual gtsam::NonlinearFactorGraph create_factors(const int current, gtsam::Values& new_values) = 0;

  virtual void fallback_smoother() {}
  virtual void update_frames(const int current, const gtsam::NonlinearFactorGraph& new_factors);

protected:
  std::unique_ptr<OdometryEstimationGazelParams> params;
